...
export function PartnerReportTable({
  data,
  onColumnOrderChange,
}: PartnerReportTableProps) {
  const { partnerColumns, partnerShowTotals } = useStore();
  const [draggedColumn, setDraggedColumn] = useState<string | null>(null);
  const [orderedColumns, setOrderedColumns] = useState<string[]>(partnerColumns);
  ...
  const formatValue = (value: any, column: string) => {
    if (typeof value === 'number') {
      const currencyColumns = ['Сумма выпущенных ЭПЛ', 'Средняя цена ЭПЛ'];
      const countColumns = ['Количество водителей', 'Количество организаций', 'Количество ЭПЛ'];
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {partnerShowTotals && data?.length > 0 && (
